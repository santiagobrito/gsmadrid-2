...
function extractPrices(precios?: { concepto: string; importe: number; nota?: string }[] | null) {
  ...
  // Match concepto by profile + optional modality
  // Examples: "Colegiado Presencial" → colegiado + presencial
  //           "Colegiado Online" → colegiado + online
  //           "Colegiado" → colegiado (all modalities)
  //           "Externo" or "General" → externo (all modalities)
  function findPrice(profile: string, modality: string): number | null {
    ...
    // First: try exact match (profile + modality)
    ...
    // Fallback: profile only (no modality specified → same price for all)
    for (const alias of profileAliases) {
